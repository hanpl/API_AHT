...
namespace AHTAPI.Repositories
{
    public class EntranceCRepository
    {
        ...
        public DataTable GetCodeShareById(string id)
        {
            ...
            "where A.Mcdt between DATEADD(Mi, -50, getdate()) and DATEADD(hh, 6,getdate()) AND A.Status<>'' and A.Status<> 'Cancelled' " +
        ...
        {
            string query = "SELECT LineCode, CONCAT(LineCode, Number) AS Flight, Mcdt, Schedule,Estimated, RowFrom, RowTo, Id FROM AHT_FlightInformation WHERE Mcdt BETWEEN DATEADD(Mi, -50, GETDATE()) AND DATEADD(hh, 6, GETDATE()) " +
              " AND Status<> '' " +
        ...
        {
            string query = "SELECT LineCode,CONCAT(LineCode, Number) AS Flight, Mcdt, Schedule, Estimated, RowFrom, RowTo, Id FROM AHT_FlightInformation WHERE Mcdt BETWEEN DATEADD(Mi, -20, GETDATE()) AND DATEADD(hh, 6, GETDATE()) " +
              " AND Status<> '' " +
              " AND Status<> 'Cancelled' " +
              " AND Adi = 'D' " +
              " AND RowFrom NOT LIKE 'M%' " +
